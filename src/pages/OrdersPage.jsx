...
export function OrdersPage() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-8 md:px-6">
      ...
      <p className="mt-1 text-sm text-neutral-500">Historial de compras y estado de envios</p>
      <div className="mt-6 space-y-4">
        {items.map((order) => (
          <article
            key={order.id}
            className="grid gap-4 rounded border border-line bg-white p-5 sm:grid-cols-[1fr_auto] sm:items-center"
          >
            <div>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <h2 className="font-extrabold">Pedido #{order.id}</h2>
                <span className={`rounded px-2 py-1 text-xs font-bold ${statusStyles[order.estadoPedido]}`}>
      ...
      {items.length === 0 && (
        <div className="mt-10 rounded border border-line bg-white p-8 text-center">
          <p className="mb-4 text-sm text-neutral-500">Todavia no tenes pedidos.</p>
          <Button to="/">Comprar ahora</Button>
        </div>
      )}
    </section>
  );
