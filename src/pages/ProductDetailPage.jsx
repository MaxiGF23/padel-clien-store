      ...
      <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <div className="mt-4 grid grid-cols-4 gap-3 sm:w-80">
            {[product, { visual: "🟡" }, { visual: "📐" }, { visual: "🎯" }].map((item, i) => (
              <button key={i} className={`rounded border bg-white p-2 ${i === 0 ? "border-forest" : "border-line"}`}>
        ...
        <div className="max-w-xl">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <Button onClick={() => dispatch(addToCart({ product, quantity }))}>Agregar al carrito</Button>
            <Button variant="secondary" onClick={() => dispatch(addToCart({ product, quantity }))}>
              Comprar ahora
            </Button>
          </div>
